...
APP  = os.path.join(BASE, "raven_weltrade.py")
...
def get_tunnel():
    ...
    _cf_proc = subprocess.Popen([CF,"tunnel","--url",f"http://localhost:{PORT}"],
        stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
        creationflags=subprocess.CREATE_NO_WINDOW, cwd=BASE)
    for raw in _cf_proc.stdout:
...
def cleanup():
    for p in (_cf_proc, _st_proc):
        if p:
            try: p.terminate()
            except Exception: pass

root = tk.Tk()
root.title("RAVEN AI · Weltrade MT5")
...
tk.Label(root, text="🌐  RAVEN AI · Weltrade MT5 Scanner",
         bg="#07070f", fg="#42a5f5", font=f_title).pack(pady=(22,3))
tk.Label(root, text="100% MetaTrader 5 — Forex · Metales · Índices · Crypto",
...
tk.Label(root, textvariable=status_var, bg="#07070f", fg="#69f0ae", font=f_st).pack()
url_var = tk.StringVar(value="")
url_lbl = tk.Label(root, textvariable=url_var, bg="#07070f", fg="#82b1ff",
                   font=f_url, cursor="hand2")
url_lbl.pack(pady=6)

btn_frame = tk.Frame(root, bg="#07070f"); btn_frame.pack(pady=4)
open_btn = tk.Button(btn_frame, text="🌐  Abrir navegador",
    command=lambda: webbrowser.open(url_var.get()) if url_var.get() else None,
    bg="#0d3a6e", fg="#fff", relief="flat", font=f_btn, padx=16, pady=6, state="disabled")
open_btn.pack(side="left", padx=6)
copy_btn = tk.Button(btn_frame, text="📋  Copiar URL",
    command=lambda: [root.clipboard_clear(), root.clipboard_append(url_var.get()),
                     status_var.set("✅  URL copiada")] if url_var.get() else None,
    bg="#1a1a30", fg="#d8d8f8", relief="flat", font=f_btn, padx=16, pady=6, state="disabled")
copy_btn.pack(side="left", padx=6)

tk.Frame(root, bg="#1a1a30", height=1).pack(fill="x", padx=30, pady=12)
tk.Label(root, text="Mantén esta ventana abierta mientras usas el scanner",
         bg="#07070f", fg="#1a1a38", font=f_sub).pack()

def launch_all():
    status_var.set("🔧  Liberando puerto 8505…")
    kill_port(); time.sleep(1)
    status_var.set("▶  Iniciando Streamlit…")
    start_streamlit()
    status_var.set("⏳  Esperando que arranque…")
    ready = wait_ready(25)
    if ready:
    ...
    tunnel = get_tunnel()
    final_url = tunnel or f"http://localhost:{PORT}"
    url_var.set(final_url)
    if tunnel:
        status_var.set("✅  Activo — accesible desde cualquier dispositivo")
        url_lbl.bind("<Button-1>", lambda e: webbrowser.open(tunnel))
    else:
        status_var.set("⚠  Túnel no disponible — solo local")
