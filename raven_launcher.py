import subprocess, sys, os, time, webbrowser, re, threading, shutil, signal
import tkinter as tk
from tkinter import font as tkfont

BASE   = r"C:\Users\saems"
APP    = os.path.join(BASE, "raven_metals_forex.py")
CF     = os.path.join(BASE, "cloudflared.exe")
PORT   = 8504

# Buscar Python del sistema (no el exe de PyInstaller)
def find_python():
    candidates = []
    # sys.executable es el propio .exe cuando se corre desde PyInstaller — NO agregarlo
    ...
    for c in candidates:
        ...
        if not os.path.isfile(c):
            continue
        # Nunca usar el propio exe de PyInstaller
        if c.lower() == sys.executable.lower():
            continue
        if c.lower().endswith(".exe") and "python" not in os.path.basename(c).lower():
            continue
        try:
            r = subprocess.run(
                [c, "--version"],
...
def wait_streamlit_ready(timeout=20):
...
def cleanup():
    global _streamlit_proc, _tunnel_proc
...
# ── UI ────────────────────────────────────────────────────────────────────────
root = tk.Tk()
root.title("RAVEN AI · Metales & Forex")
...
# Widgets
tk.Label(root, text="🥇  RAVEN AI · Metales & Forex",
         bg="#07070f", fg="#ffd600", font=f_title).pack(pady=(22, 3))
tk.Label(root, text="Scanner de señales en tiempo real — XAUUSD · FOREX",
...
tk.Label(root, textvariable=status_var,
         bg="#07070f", fg="#69f0ae", font=f_status).pack()

url_var  = tk.StringVar(value="")
url_lbl  = tk.Label(root, textvariable=url_var,
                    bg="#07070f", fg="#82b1ff", font=f_url,
                    cursor="hand2")
url_lbl.pack(pady=6)

btn_frame = tk.Frame(root, bg="#07070f")
...
open_btn = tk.Button(btn_frame, text="🌐  Abrir en navegador",
                     command=open_browser,
                     bg="#3d1278", fg="#fff", relief="flat",
                     font=f_btn, padx=16, pady=6, state="disabled")
open_btn.pack(side="left", padx=6)

copy_btn = tk.Button(btn_frame, text="📋  Copiar URL",
                     command=copy_url,
                     bg="#1a1a30", fg="#d8d8f8", relief="flat",
                     font=f_btn, padx=16, pady=6, state="disabled")
...
def launch_all():
    ...
    status_var.set("⏳  Esperando que Streamlit arranque…")
    ready = wait_streamlit_ready(timeout=25)

    if ready:
        webbrowser.open(f"http://localhost:{PORT}")
        status_var.set("🌐  Conectando túnel Cloudflare…")
    else:
        status_var.set("⚠  Streamlit tardó — intentando túnel igual…")
    ...
    if tunnel_url:
        ...
        status_var.set("⚠  Túnel no disponible — funcionando solo en local")
        open_btn.config(state="normal")
